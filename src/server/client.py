        ...
        async with websockets.connect(
            ...
            async for raw_msg in ws:
                ...
                if etype == "heartbeat":
                    ...
                    for s in steps:
                        icon = {"think":"💭","code":"💻","analyze":"🔍",
                                "validate":"✅","summarize":"📝"}.get(s.get("step_type",""), "•")
                    ...
                    ok(f"Task finished successfully")
...
def test_evaluate():
    ...
    def grade(val):
        if val is None: return f"{DIM}N/A{RESET}"
...
async def run_single_test(test_name: str):
    ...
    if not check_server_running():
        ...
        print(f"  Start it with: python -m src.api.main")
        sys.exit(1)
